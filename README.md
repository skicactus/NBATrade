...
- **`src/engine/`** — pure functions, no I/O, no framework dependency. `playerValue` models surplus value (market value vs. actual salary, discounted per remaining contract year), scaled by an age curve that also carries a "cornerstone" upside premium — a young player already performing at a starter-or-better level (a Wembanyama, a No. 1 pick rookie) is worth meaningfully more than the identical box score from a player already in their prime, since that's what real front offices pay for; a young player who isn't yet producing gets the plain age curve only, not a speculative bonus. `checkSalaryMatch` implements the real NBA salary-matching bands by apron status; `evaluateTrade` combines both for a specific proposed trade; `findTrades` runs the combinatorial search behind the trade finder. Fully covered by `vitest` — these are the functions a resume bullet points at.
